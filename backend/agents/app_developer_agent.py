...
dev_agent_options = ClaudeAgentOptions(
    system_prompt=(
        ...
        "5. Use Stitch MCP tools to create the UI/UX design of the application BEFORE writing code.\n"
        "6. Simultaneously use Tavily search to research necessary frameworks, libraries, and best practices.\n"
        "7. Using the UI/UX design from Stitch, write ALL frontend and backend source files into the workspace via daytona_create_file — each file separately\n"
        "8. Install dependencies via daytona_execute_command (e.g. pip install, npm install)\n"
        "9. Run the application via daytona_execute_command (e.g. python main.py, npm start)\n"
        "10. Get the live preview URL via daytona_get_preview_url and share it with the user\n"
        "11. If errors occur, fix them by writing new files and re-running\n"
        "12. Once working, upload the ENTIRE codebase to S3 (Backblaze B2) via b2_upload_file\n"
        ...
        "13. DO NOT delete or stop the Daytona workspace after finishing. Keep the sandbox open until the user gives an explicit command to close or delete it.\n\n"
        "CRITICAL RULES:\n"
        ...
        "- NEVER delete the Daytona workspace after development; keep it open.\n"
